...
# Build cross compiler
{
    ...
    [ -z "$(command -v "${cc_target}-as")" ] && {
        error "binutils did not build properly - '${cc_target}-as' not found"
        exit 1
    }
    ...
    [ -z "$(command -v "${cc_target}-gcc")" ] && {
        error "gcc did not build properly - '${_target}-gcc' not found"
        exit 1
    }
    ...
    if ! (printf '%s' 'int main(){return 0;}' | "${cc_target}-gcc" -x c - -o "${_testbin}" -Wl,--verbose > "${_testlog}" 2>&1); then
        error ">>> gcc failed to compile simple test program, dumping log"
        cat "${_testlog}"
        exit 1
    fi
    ...
    if ! (printf '%s' "${_libc}" | grep -q "/lib/ld-musl-$(uname -m).so.*"); then
        error ">>> gcc failed to link to musl libc"
        cat "${_testlog}"
        exit 1
    fi
...
# Mark stage finished
{
    note "ignore following errors generated by 'strip' program"
